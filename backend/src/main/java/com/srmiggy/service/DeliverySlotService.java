...
public class DeliverySlotService {
    ...
    private static final LocalTime ORDERING_START_TIME = LocalTime.of(8, 0);
    private static final LocalTime ORDERING_END_TIME = LocalTime.of(21, 0);

    /**
     * Get available slots based on current time
     * Returns slots that start after the current time and before 9:00 PM
     * Returns empty list if current time is after 9:00 PM or before 8:00 AM
     */
    public List<DeliverySlot> getAvailableSlots() {
        LocalTime now = LocalTime.now();
        
        // If current time is after 9:00 PM or before 8:00 AM, no slots are available
